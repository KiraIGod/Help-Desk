...
export class TicketsService {
  ...
  async findAll(filters: FilterTicketsDto): Promise<PaginatedTicketResponseDto> {
    ...
    try {
      ...
      this.logger.warn('cache_get_failed', 'TicketsService', { key: cacheKey, error: (err as Error)?.message });
    }
    ...
    try {
      ...
      this.logger.warn('cache_set_failed', 'TicketsService', { key: cacheKey, error: (err as Error)?.message });
    }
